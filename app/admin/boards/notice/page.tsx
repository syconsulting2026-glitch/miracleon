...
import { useDeleteManyNotices, useNotices } from "@/hooks/useNotices";
...
export default function AdminNoticePage() {
  ...
  const deleteManyMutation = useDeleteManyNotices();

  const notices = data?.items ?? [];
  const totalPages = data?.totalPages ?? 1;

  const allVisibleIds = useMemo(() => notices.map((item) => item.id), [notices]);
  const isAllSelected =
        ...
        <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
          <section className="space-y-6">
            <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  공지사항 목록을 확인하고 등록, 선택 삭제를 할 수 있습니다.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="inline-flex h-11 items-center justify-center rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-semibold text-red-600 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  선택 삭제
                </button>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="hidden grid-cols-[60px_100px_1fr_140px_100px] bg-gray-100 px-6 py-4 text-sm font-semibold text-gray-700 md:grid">
              ...
              <div className="divide-y divide-gray-200">
                {isLoading ? (
                  ...
                  notices.map((item) => (
                    ...
                    >
                      <div className="grid grid-cols-1 gap-3 md:grid-cols-[60px_100px_1fr_140px_100px] md:items-center">
                        ...
                        <div className="min-w-0">
                          <Link
                            href={`/admin/boards/notice/${item.id}`}
